...
function sector_t.render(
	...
	for wallid = 1, #self do
		...
		local p_left, p_right =
			math.round(f_left * halfwidth) + 1 + halfwidth, math.round(f_right * halfwidth) + 1 + halfwidth
		...
		flb, frb, clt, crt =
			flb * halfheight + halfheight + 1,
			frb * halfheight + halfheight + 1,
			clt * halfheight + halfheight + 1,
			crt * halfheight + halfheight + 1
		flt, frt, clb, crb =
			flt * halfheight + halfheight + 1,
			frt * halfheight + halfheight + 1,
			clb * halfheight + halfheight + 1,
			crb * halfheight + halfheight + 1
		...
		for c = left_edge + 1, right_edge do
			if top[c] > bottom[c] then
				local step = c - p_left
				local fb, ft, ct, cb, y =
					math.floor(fbs * step + flb),
					math.floor(fts * step + flt),
					math.floor(cts * step + clt),
					math.floor(cbs * step + clb),
					ys * step + a.y
